...
async function loadconstDB() {
    try {
        const res1 = await authedFetch(`/v2.0/apps/constdb/datasets?dbid=${route.params.databaseid}`, {
            method: 'GET',
        ...
        let ids = []
        constDBinfo.value.forEach((pointfull) => {
            if (pointfull.coords !== null && !ids.includes(pointfull.siteid)) {
                const point = pointfull.coords
...
onMounted(async () => {
    ...
    var displayFeatureInfo = async function (pixel, coordinate) {
        var features2 = []
        names.value = []
        ids.value = []
        coords.value = []
...
<template>
    <div>
        ...
        <div v-if="uniquenames.length == 1">
            ...
            >
                ...
                <div v-for="(el, index) in link">
                    <span
                        ><a :href="link[index]" target="_blank">{{ datasets[index] }}</a> ({{ datatypes[index] }})</span
                    >
                </div>
                <p>{{ descrips[0] }}</p>
            </Dialog>
        ...
        <div v-if="uniquenames.length > 1 && uniquenames.length < threshold">
            <Dialog
                header="Multiple sites with same location"
                v-model:visible="visible"
                modal
                :style="{ width: '50rem' }"
                :breakpoints="{ '1199px': '75vw', '575px': '90vw' }"
            >
                ...
                <div v-for="(site, index) in Object.keys(dataindexer)">
                    ...
                    <span v-for="el in dataindexer[site]">
                        <span>
                            <a :href="link[el - 1]" target="_blank">{{ datasets[el - 1] }}</a> ({{
                                datatypes[el - 1]
                            }}),
                        </span>
                    </span>
        ...
        <div v-if="uniquenames.length >= threshold">
            <Dialog
                header="Many many sites with same coordinates"
                v-model:visible="visible"
                modal
                :style="{ width: '50rem' }"
                :breakpoints="{ '1199px': '75vw', '575px': '90vw' }"
