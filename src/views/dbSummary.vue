    ...
    const filtered = databasekeys.value.filter((car) => {
        ...
        if (car.datasettypes != null) {
            matchData = Object.values(datafilter.value).every((filter) =>
                car.datasettypes.some(
                    (dataset) => dataset.datasettype !== null && dataset.datasettype.toString() === filter
                )
            )
        } else {
            return false
...
<template>
    <Panel toggleable>
        ...
        <p style="font-size: 20px">
            Neotoma is a database of databases. Each constituent database in Neotoma specializes in the curation of
            paleoecological data from some particular configuration of time, space, and proxy type. For instance, the
            main focus of the North American Pollen Database is on late Quaternary pollen records from North America (as
            the name suggests!).
        </p>
        <p style="font-size: 20px">
            Although Neotoma's constituent databases specialize in particular kinds of paleo data, they tend over the
            years to have accumulated other data which arguably fall outside their ambit. (If you explore the North
            American Pollen Database, you will notice it also includes records from other continents and concerning a
            variety of proxies.) The landing pages for Neotoma's constituent databases (linked below) are meant to
            facilitate data exploration by constituent database, in order to make the sometimes unexpected holdings of
            any given database more transparent to Neotoma's users. These pages provide database-specific information on
            dataset types, spatial and temporal coverage, upload history, and data contributors.
        </p>
        <p style="font-size: 20px">
            On this page, you may filter by database name, dataset type, and temporal coverage, to find Neotoma
            constituent databases that may be of interest to you. To explore any database in more detail, simply click
            its button below.
        </p>
    </Panel>
    ...
    <Panel toggleable collapsed>
        <template #header>
            ...
            <i
                ...
                style="width: 10px; font-size: 1rem; color: rgb(108, 97, 71)"
                v-tooltip="{
    ...
    <Panel toggleable collapsed>
        ...
        <div style="display: flex; flex-wrap: wrap">
            <div v-for="el in time_master">
                <Badge
                    :class="{ clicked: el.clicked, not: !el.clicked }"
                    @click="buttonfiltertime(el)"
                    :value="el.value"
                >
                </Badge>
            </div>
    ...
    <Panel>
        ...
        <div class="flex flex-wrap">
            <div v-for="(el, index) in filteredDBs.value" class="col-4">
                <div
                    v-if="el.string != '' && el.marginal_value == false && el.marginal_valuet == false"
                    style="height: 100%"
                >
                    <Button
                        style="
                        ...
                        @click="gotodb(el)"
                        :aria-label="el.database.databasename"
                    >
                        <p style="font-size: 20px; color: rgb(108, 97, 71); font-weight: bold">
                            {{ el.database.databasename }}
                        </p>
                    </Button>
                </div>
                <div
                    v-if="el.string != '' && (el.marginal_value == true || el.marginal_valuet == true)"
                    style="height: 100%"
                >
                    <Button
                        style="
                        ...
                        :aria-label="el.database.databasename"
                    >
                        <p style="font-size: 20px; color: rgb(50, 50, 30); font-weight: bold">
                            {{ el.database.databasename }}
                        </p>
                    </Button>
                </div>
                <div v-if="el.string == ''" style="height: 100%">
                    <Button
                        ...
                        :aria-label="el.database.databasename"
                    >
                        <p style="font-size: 20px; color: rgb(150, 157, 151); font-weight: bold">
                            {{ el.database.databasename }}
                        </p>
                    </Button>
                </div>
