...
<script setup>
  ...
  async function dataciteMeta(doi) {
    const response = await fetch(dataciteapi + doi, {
        ...
        const json = await response.json();
        datasetmeta.value = json['data']['attributes']
      }

  onMounted(async() => {
    await router.isReady()
    await getDatasetDOI()
    await dataciteMeta(datasetdoi.value);
  });
...
<template>
    <Panel>
        <template #header><h1>This Dataset Has Been Removed</h1></template>
        <p>This dataset has been removed from Neotoma, or a new dataset ID and DOI has been added to supercede this record.</p>
        <p>To find the new dataset, you can examine the DOI metadata and search by sitename, contributor or dataset 
            type using <a href='https://apps.neotomadb.org/explorer'>Neotoma Explorer</a>, the <a href='https://cran.r-project.org/web/packages/neotoma2/index.html'>neotoma2 R package</a> or the <a href='https://api.neotomadb.org'>Neotoma API</a>.</p>
