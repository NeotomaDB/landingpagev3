...
<template>
  <div v-if="!loadingnat">
      ...
      <div id="griddiv_nlm" style="display:grid;grid-template-columns:1fr 1fr;">
        ...
        <div v-if="hasFeatures" style="font-size:13px; ">
          ...
          <span v-for="(feat, index) in vectorSource2.getFeatures()">
            <span
              v-if="(index + 1) != vectorSource2.getFeatures().length && (index + 2) != vectorSource2.getFeatures().length">
          ...
          <span>, according to the <a href="https://native-land.ca/" target="_blank">Native Land</a> mapping
            project.</span>
          <p>Neotoma’s stewards and users come from all over the world. Wherever we are and wherever we collect and
            curate
            data,
          ...
          <p>We respect the sovereign status of the Indigenous peoples from whose lands our data derive,
            ...
            <a href="https://eos-rcn.github.io/web/home" target="_blank">Ethical Open Science research coordination
              network</a>,
            in order to work toward more ethical stewardship of Indigenous data.
          </p>
          <p>This language has been borrowed in part from <a target="_blank" href="https://localcontexts.org/">Local
              Contexts</a>.</p>
        </div>
      </div>
    </Panel>
    <div v-if="features.length == 1">
      <Dialog v-model:visible="visible" modal :header="info[0]" :style="{ width: '50rem' }"
    ...
    <div v-if="features.length > 1">
      <Dialog header="Multiple Lands" v-model:visible="visible" modal :style="{ width: '50rem' }"
        :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
